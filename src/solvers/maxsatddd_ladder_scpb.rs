...
/// Initial upper bound for SCPB k_max. Will be extended if needed.
const INITIAL_K_MAX: usize = 1000;
pub fn solve_debug<L: satcoder::Lit + Copy + std::fmt::Debug>(
    ...
    let mut objective_scpb: SCPB<L> = SCPB::new_empty(INITIAL_K_MAX);
    let mut best_ub: usize = INITIAL_K_MAX;
    ...
    loop {
            ...
            if let Some((sol_tx, sol_rx)) = heur_thread.as_ref() {
                ...
                while let Ok((ub_cost, ub_sol)) = sol_rx.try_recv() {
                    ...
                    if ub_cost < best_heur.as_ref().map(|(c, _)| *c).unwrap_or(i32::MAX) {
                        ...
                        // Update SCPB upper bound from heuristic
                        best_ub = best_ub.min(ub_cost as usize);
                    }
                }
            }
        ...
        // SCPB-based approach: no Soft enum needed.
        // Cost literals are collected into the SCPB directly.
        ...
        for (visit, new_timepoint_var, new_t) in new_time_points.drain(..) {
            ...
            if new_timepoint_cost > 0 {
                ...
                // Use CostTree and feed new cost variables into SCPB
                // Collect new cost vars in buffer first (can't borrow solver twice)
                let mut new_cost_vars: Vec<(usize, Bool<L>)> = Vec::new();
                occupations[visit].cost_tree.add_cost(
                    &mut solver,
                    new_timepoint_var,
                    new_timepoint_cost,
                    &mut |weight, cost_var| {
                        new_cost_vars.push((weight, cost_var));
                    },
                );
                // Now add collected cost vars to SCPB
                for (weight, cost_var) in new_cost_vars {
                    objective_scpb.add_input(&mut solver, cost_var, weight);
                }
            }
        ...
        log::info!(
            "solving it{} with {} timepoints {} conflicts {} cost_vars",
            iteration,
            n_timepoints,
        ...
        // Binary search for optimal cost using SCPB assumptions
        let mut lo: usize = total_cost as usize;
        let mut hi: usize = best_ub;
        let mut found_sat = false;

        let core = loop {
            let k = if !found_sat {
                // First try with current upper bound to get a feasible solution
                hi
            } else {
                // Binary search
                (lo + hi) / 2
            ...
            // Build assumption: sum <= k, i.e., !R[n-1][k] (0-indexed)
            let scpb_assumption = objective_scpb.at_most_assumption(k);

            let solve_start = Instant::now();
            let result = {
                let _p = hprof::enter("sat check");
                if let Some(assumption) = scpb_assumption {
                    SatSolverWithCore::solve_with_assumptions(
                        &mut solver,
                        std::iter::once(assumption),
                    )
                } else {
                    // k >= total possible weight, no constraint needed
                    SatSolverWithCore::solve_with_assumptions(
                        &mut solver,
                        std::iter::empty(),
                    )
            ...
            match result {
                satcoder::SatResultWithCore::Sat(model) => {
                    ...
                    // Evaluate actual cost from model
                    let actual_cost = objective_scpb.eval_cost(&|b| model.value(&b));
                    println!("    SAT with bound k={}, actual_cost={}", k, actual_cost);
                    ...
                    if lo >= hi {
                        // Optimal found
                        total_cost = hi as i32;
                        println!("    OPTIMAL cost = {}", total_cost);
                    }
                ...
                satcoder::SatResultWithCore::Unsat(_core) => {
                    stats.n_unsat += 1;
                    // SCPB UNSAT means cost > k, tighten lower bound
                    println!("    UNSAT with bound k={}", k);
                    lo = k + 1;

                    if lo >= hi {
                        if found_sat {
                            // We had a SAT solution and now proved optimality
                            total_cost = hi as i32;
                            println!("    OPTIMAL cost = {} (proven)", total_cost);
                            break None;
                        } else {
                            // Never found SAT - problem is infeasible at current UB
                            is_sat = false;
                            break Some(());
                        }
                    }

                    // Continue binary search - don't break
                    continue;
                }
            }
        };
        ...
        if let Some(()) = core {
            // Never found a feasible solution, possibly need to increase k_max
            // or problem is truly UNSAT
            if objective_scpb.is_empty() {
                return Err(SolverError::NoSolution);
            }
            // Increase UB and try again
            best_ub = best_ub * 2;
            println!("    Increasing UB to {}", best_ub);
            // Need to rebuild SCPB with larger k_max - for now just
            // let the DDD loop continue with new constraints
